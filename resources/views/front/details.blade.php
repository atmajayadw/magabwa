	...
	<section id="Article-container" class="max-w-[1130px] mx-auto flex gap-20 mt-[50px]">
		...
		<div class="side-bar flex flex-col w-[300px] shrink-0 gap-10">
			<div class="ads flex flex-col gap-3 w-full">
				<a href="{{$bannersquare_1->link}}">
					<img src="{{Storage::url($bannersquare_1->thumbnail)}}" class="object-contain w-full h-full" alt="ads" />
				</a>
				...
				<a href="{{route('front.details', $news->slug)}}" class="card-from-author">
					<div
						...
						<div class="flex flex-col gap-[6px]">
							...
							<p class="text-xs leading-[18px] text-[#A3A6AE]">{{$news->created_at->format('M d, Y')}} • {{ strtoupper($news->category->name) }}</p>
						</div>
					</div>
				</a>
                @empty
			...
			<div class="ads flex flex-col gap-3 w-full">
				<a href="{{$bannersquare_2->link}}">
					<img src="{{Storage::url($bannersquare_2->thumbnail)}}" class="object-contain w-full h-full" alt="ads" />
				</a>
				<p class="font-medium text-sm leading-[21px] text-[#A3A6AE] flex gap-1">
					Our Advertisement <a href="#" class="w-[18px] h-[18px]"><img
							src="{{asset('assets/images/icons/message-question.svg')}}" alt="icon" /></a>
